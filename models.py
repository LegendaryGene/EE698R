...
class Decoder(nn.Module):
  def __init__(self):
    ...
    self.fc1=nn.Linear(256,32*16*256)
    self.bn1=nn.BatchNorm1d(32*16*256,momentum=0.9)
    self.relu=nn.LeakyReLU(0.2)
    ...
    self.deconv4=nn.Conv2d(32,3,5, stride=1, padding=2)
    self.tanh=nn.Tanh()

  def forward(self,x):
    batch_size=x.size()[0]
    x=self.relu(self.bn1(self.fc1(x)))
    x=x.view(batch_size,256,32,16)
...
class Discriminator(nn.Module):
  def __init__(self):
    ...
    self.bn3=nn.BatchNorm2d(256,momentum=0.9)
    self.fc1=nn.Linear(32*16*256,512)
...
class VAE_GAN(nn.Module):
  ...
  def forward(self,x,addendum=None):
    bs=x.size()[0]
    z_mean,z_logvar=self.encoder(x)
    std = z_logvar.mul(0.5).exp_()
    if addendum is not None:
        z_mean=z_mean+addendum
        
    epsilon=Variable(torch.randn(bs,256)).to(device)
